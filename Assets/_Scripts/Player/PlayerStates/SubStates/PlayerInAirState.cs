...
public class PlayerInAirState : PlayerState
{
    ...
    public override void LogicUpdate()
    {
        ...
        if (Movement?.CurrentVelocity.y > 0.05f)
        {
            playerData.isJumping = true;
        }
        else if (Movement?.CurrentVelocity.y <= 0.05f || isGrounded || jumpInputStop)
        {
        ...
        else if (isTouchingWall && (xInput == Movement?.FacingDirection || playerData.wallJumpCombo) && playerData.canStickWall && (Movement?.CurrentVelocity.y <= 0f || playerData.wallJumpCombo))
        {
